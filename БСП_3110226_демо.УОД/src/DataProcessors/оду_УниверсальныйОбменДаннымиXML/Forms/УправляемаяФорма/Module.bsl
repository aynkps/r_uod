...
&НаКлиенте
Процедура ТаблицаНастройкиПараметровЗначениеОчистка(Элемент, СтандартнаяОбработка)
	 Элемент.ВыбиратьТип = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаНастройкиПараметровЗначениеПриИзменении(Элемент)
	
	ТекЭлемент = Элементы.ТаблицаНастройкиПараметров.ТекущиеДанные;
	
	Если ТекЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ДокСсылка = ТекЭлемент.Значение;
	ТаблицаНастройкиПараметровЗначениеПриИзмененииНаСервере(ТекЭлемент.Наименование, ДокСсылка);
	
КонецПроцедуры

&НаСервере
Процедура ТаблицаНастройкиПараметровЗначениеПриИзмененииНаСервере(ИмяПараметра, ДокСсылка)
	
	ЭтоДокумент = Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(ДокСсылка));
	Если ЭтоДокумент = Ложь  Тогда
		Возврат;
	КонецЕсли; 
	Если  ИмяПараметра = "Док_Ссылка"  Тогда
		Организация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокСсылка, "Организация");	
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Организация)  Тогда
		Мстр = Объект.ТаблицаНастройкиПараметров.НайтиСтроки(Новый Структура("Наименование", "Организация"));
		Если Мстр.Количество() > 0 Тогда
			НСтр = Мстр[0];
			НСтр.Значение = Организация;
		КонецЕсли; 
	КонецЕсли; 
