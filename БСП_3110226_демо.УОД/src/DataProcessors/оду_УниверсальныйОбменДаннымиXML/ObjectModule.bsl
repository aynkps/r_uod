...
//TODO:  оду_Доработка ищет имя по типу метаданных, а в правиле может быть тот же тип, но разные использования поиска
Функция ОпределитьПоТипуОбъектаИспользоватьДополнительныйПоискПоПолямПоиска(ТипСсылкиСтрокой, ИмяПКО = "")
	
	//ТипСсылкиСтрокой = ?(ИмяПКО = "", ТипСсылкиСтрокой, ИмяПКО);
		...
		Для Каждого Элемент Из Правила Цикл
			...
			Если ИмяПКО = "" Тогда
				Если Элемент.Значение.Приемник = ТипСсылкиСтрокой Тогда
					оду_НашлиПравило = Истина;
				КонецЕсли;               
			ИначеЕсли Элемент.Ключ = ИмяПКО Тогда
				оду_НашлиПравило = Истина;
			Иначе
				оду_НашлиПравило = Ложь;
			КонецЕсли;               
			
			//Если Элемент.Значение.Приемник = ТипСсылкиСтрокой Тогда
			Если оду_НашлиПравило = Истина Тогда
...
Функция НайтиОбъектПоСсылке(ТипОбъекта,
	...
	ИначеЕсли (СвойствоУникальныйИдентификатор <> Неопределено) Тогда
		...
		НужноПродолжитьПоискЕслиЭлементПоGUIDНеНайден = ОпределитьПоТипуОбъектаИспользоватьДополнительныйПоискПоПолямПоиска(СтруктураСвойств.ТипСсылкиСтрокой, ИмяПКО);
		...
		ИначеЕсли ИмяУзла = "Ссылка" Тогда
			...
			Иначе
				...
				Если Ссылка = Неопределено Тогда
					...
					// ▼ ayan#мпп#bitrix2289 Быстрый и эффективный обмен УТАП - УТ - УТАП  КирилловПС  2023.06.05 ▼ Начало
					оду_ДобавитьВСписокНеЗаписанныхОбъектов(ИзвестнаяСсылкаУникальногоИдентификатора, Правило);
					// ▲ ayan#мпп#bitrix2289 Быстрый и эффективный обмен УТАП - УТ - УТАП  КирилловПС  2023.06.05 ▲ Конец 
...
Процедура ВыполнитьЗагрузку(оду_РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
	...
	Попытка
		...
		Если оду_ДокументыДляОтложенногоПроведения <> Неопределено Тогда
	...
	Исключение 
		...
		// ▼ ayan#мпп#bitrix2289 Быстрый и эффективный обмен УТАП - УТ - УТАП  КирилловПС  2023.06.05 ▼ Начало
		оду_ЗафиксироватьОшибку(, СтрокаСообщенияОбОшибке);
		// ▲ ayan#мпп#bitrix2289 Быстрый и эффективный обмен УТАП - УТ - УТАП  КирилловПС  2023.06.05 ▲ Конец 
	...
	Попытка
		...
		Если Не Отказ Тогда
			...
			Если Не Отказ Тогда
				...
				//оду
				Если Комментарий = "оду_ВыгрузкаИзРегистрации" ИЛИ Комментарий = "оду_ВыгрузкаДанныхИзПланаОбмена"  Тогда
					УдачноВыгруженоПоПланамОбмена = оду_ОбработатьПравилаВыгрузки();
					Если УдачноВыгруженоПоПланамОбмена = Ложь Тогда
						ВызватьИсключение "*оду. Ошибка выгрузки данных";	
					КонецЕсли;
				Иначе
...
Функция оду_ВыполнитьВыгрузкуПоДанным()
	...
	Попытка
		...
		Если Комментарий = "оду_ВыгрузкаДанныхИзПланаОбмена" Тогда
			оду_ЗаполнитьПараметры("оду_УзелПланаОбмена", Данные.оду_УзелПланаОбмена);
		...
		Для каждого Стр Из СоответвиеПараметровВыгрузки Цикл
			...
			//оду_ИмяПКО = СтрРазделить(оду_ТипСсылки, ".")[1];
			//оду_ЗаполнитьПараметры("оду_ТипСсылки", оду_ИмяПКО);
			
			оду_ЗаполнитьПараметры("оду_ТипСсылки", оду_ТипСсылки);
			оду_ЗаполнитьПараметры("оду_МассивСсылок1С", оду_СтруктураДанных.оду_МассивСсылок1С);
			Если оду_СтруктураДанных.Свойство("оду_ДополнительныеСвойстваОбъекта") Тогда
				оду_ЗаполнитьПараметры("оду_ДополнительныеСвойстваОбъекта", оду_СтруктураДанных.оду_ДополнительныеСвойстваОбъекта);
			КонецЕсли;
			//оду_ЗаполнитьПараметры("оду_ДополнительныеСвойстваОбъектаПриемник", оду_СтруктураДанных.оду_ДополнительныеСвойстваОбъекта);
	...
	Исключение
		...
		оду_Общий.ОтправитьСообщениеВТелеграмExchange("УниверсальныйОбменДаннымиXML", , пр_Ошибка);
...
Функция оду_РезультатДобаленияОбъектаВСписокНеЗаписанныхОбъектов(СсылкаНаОбъект, ИзвестнаяСсылкаУникальногоИдентификатора) Экспорт
	...
	Исключение
		СтрокаСообщенияОбОшибке = ОписаниеОшибки();
		оду_Общий.ОтправитьСообщениеВТелеграмExchange("оду_УниверсальныйОбменДаннымиXML.оду_РезультатДобаленияОбъектаВСписокНеЗаписанныхОбъектов", , СтрокаСообщенияОбОшибке);	
...
Процедура оду_ДобавитьВСписокНеЗаписанныхОбъектов(оду_Ссылка, Правило)
	
	Попытка
		
		оду_ИмяПравила = Правило.Имя;
		//Если Лев(ИмяПравила,2) = "л_" Тогда
		Если Лев(оду_ИмяПравила,3) = "зс_" Тогда  //Запрос ссылки
			//ИЛИ Лев(ИмяПравила,2) = "л_" Тогда  //Запрос ссылки  
			оду_СтруктураОбъекта = Новый Структура("оду_Ссылка, оду_ТипСсылки", оду_Ссылка, оду_ТипСсылки(Правило.Источник));	
			оду_СтекНеЗаписанныхОбъектов.Добавить(оду_СтруктураОбъекта);	
		КонецЕсли; 
...
Функция оду_ТипСсылки(оду_ТипСсылкиНачало)
	
	Попытка
		
		оду_МассивТипСсылки = СтрРазделить(оду_ТипСсылкиНачало, ".", Ложь); 
		оду_ТипСсылкиИсточник = оду_МассивТипСсылки[0];                     
		
		Если оду_ТипСсылкиИсточник = "СправочникСсылка" Тогда
			оду_ТипСсылки = "CatalogRef";		
		ИначеЕсли оду_ТипСсылкиИсточник = "ДокументСсылка" Тогда  
			оду_ТипСсылки = "DocumentRef";		
		ИначеЕсли оду_ТипСсылкиИсточник = "ПланВидовХарактеристикСсылка" Тогда
			оду_ТипСсылки = "ChartOfCharacteristicTypesRef";		
		Иначе 
			оду_ТипСсылки = "";		
		КонецЕсли;    
		
		Если оду_ТипСсылки <> "" Тогда
			оду_ТипСсылки = СтрШаблон("%1.%2", оду_ТипСсылки, оду_МассивТипСсылки[1]); 	
		КонецЕсли;
		
		Возврат оду_ТипСсылки;
		
	Исключение
		СтрокаСообщенияОбОшибке = ОписаниеОшибки();
		оду_Общий.ОтправитьСообщениеВТелеграмExchange("оду_УниверсальныйОбменДаннымиXML.оду_ТипСсылки", , СтрокаСообщенияОбОшибке);	
		Возврат "";
	КонецПопытки;
...
Процедура оду_ЗаписатьНеЗаписанныеОбъекты()
	...
	//Попытка   
		...
		Для каждого оду_СтруктураОбъекта Из оду_СтекНеЗаписанныхОбъектов Цикл
		//Для каждого СтруктураПараметров Из оду_СтекНеЗаписанныхОбъектов Цикл
			//оду_Ссылка = СтруктураПараметров.ИзвестнаяСсылка; 
			
			оду_Ссылка = оду_СтруктураОбъекта.оду_Ссылка;
			
			Если НЕ пр_Общий.СсылкаСуществует(оду_Ссылка)  Тогда                                                           
				пр_Ошибка = "Нет ссылки не объект в оду_СтекНеЗаписанныхОбъектов";
				оду_Общий.ОтправитьСообщениеВТелеграмExchange("оду_УниверсальныйОбменДаннымиXML.оду_ЗаписатьНеЗаписанныеОбъекты", оду_Ссылка, пр_Ошибка);	
				Продолжить;			
			КонецЕсли; 
			...
			Если ТипЗнч(Данные) = Тип("Структура") Тогда
				Если Данные.Свойство("ИДБазыИсточник") Тогда
					Нстр.ИДБазыИсточник = Данные.ИДБазыИсточник;
				КонецЕсли;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(Нстр.ИДБазыИсточник) Тогда
				Если Параметры.Свойство("оду_СтруктураУО") Тогда
					Если Параметры.оду_СтруктураУО = "" Тогда
						пр_Ошибка = "Нет данных по полю: ИДБазыИсточник в оду_СтруктураУО";
						оду_Общий.ОтправитьСообщениеВТелеграмExchange("оду_УниверсальныйОбменДаннымиXML.оду_ЗаписатьНеЗаписанныеОбъекты", оду_Ссылка, пр_Ошибка);	
						Продолжить;			
					КонецЕсли;
					оду_СтруктураУО = ЗначениеИзСтрокиВнутр(Параметры.оду_СтруктураУО);	 
					Нстр.ИДБазыИсточник = оду_СтруктураУО.ИДБазыИсточник; 
					
					Если НЕ ЗначениеЗаполнено(Нстр.ИДБазыИсточник) Тогда 
						пр_Ошибка = "Нет данных по полю: ИДБазыИсточник";
						оду_Общий.ОтправитьСообщениеВТелеграмExchange("оду_УниверсальныйОбменДаннымиXML.оду_ЗаписатьНеЗаписанныеОбъекты", оду_Ссылка, пр_Ошибка);	
						Продолжить;			
					КонецЕсли;
			...
			//	КонецЕсли;
			//	оду_СтруктураУО = ЗначениеИзСтрокиВнутр(Параметры.оду_СтруктураУО);	
			//	ЗаполнитьЗначенияСвойств(Нстр, оду_СтруктураУО); 
			//	
			...
			Нстр.ИДБазыПриемник = НастройкиТекущейБД.ИДБазыИсточник;
			Нстр.ИДОбъекта = XMLСтрока(оду_Ссылка); 
			
			Если ЗначениеЗаполнено(оду_СтруктураОбъекта.оду_ТипСсылки) Тогда
				Нстр.ТипXMLОбъектаИсточника = оду_СтруктураОбъекта.оду_ТипСсылки;	
			Иначе
		...
		Запрос.Текст = 
			...
			|	ЕСТЬNULL(оду_БазыУчастникиОбменаСостав.ТипXMLОбъектаПриемника, ВТ_Данные.ТипXMLОбъектаИсточника) КАК ТипXMLОбъектаИсточника,
			|	ВТ_Данные.ТипXMLОбъектаИсточника КАК ТипXMLОбъектаПриемника,
			|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаРегистрации,
			|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаВыгрузки,
			|	"""" КАК ИмяФайлаОбмена,
			//|	&ИмяФайлаОбмена КАК ИмяФайлаОбмена,
			...
			|	&ДополнительныеСвойстваОбъекта КАК ДополнительныеСвойстваОбъекта
			|ИЗ
			...
			|			И (оду_СостояниеОбъектовОбмена.ИДБазыИсточник = ВТ_Данные.ИДБазыИсточник)
			|			И (оду_СостояниеОбъектовОбмена.РежимОбмена = &РежимОбменаЗапросСсылки)
			...
			//|		ПО ВТ_Данные.ТипXMLОбъектаИсточника = оду_БазыУчастникиОбменаСостав.ТипXMLОбъектаПриемника
			//|			И (оду_БазыУчастникиОбменаСостав.Ссылка.ИДБазы = ВТ_Данные.ИДБазыИсточник) 
			|			И (оду_БазыУчастникиОбменаСостав.Ссылка.ИДБазы = ВТ_Данные.ИДБазыПриемник)
			|ГДЕ
		...
		оду_ДополнительныеСвойстваОбъекта = Новый Структура("оду_ЗапросСсылки", Истина);
		Запрос.УстановитьПараметр("ДополнительныеСвойстваОбъекта", ЗначениеВСтрокуВнутр(оду_ДополнительныеСвойстваОбъекта));
		...
		Пока ВДЗ.Следующий() Цикл
			...
			Запись.ИмяФайлаОбмена = ИмяФайлаОбмена;	
			Запись.Записать();
		КонецЦикла;
